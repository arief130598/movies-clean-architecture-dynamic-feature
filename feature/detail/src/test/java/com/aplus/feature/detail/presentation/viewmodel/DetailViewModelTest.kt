...
import com.aplus.feature.detail.MainCoroutineRule
import com.aplus.feature.detail.TestDispatcherProvider
import com.aplus.feature.detail.repository.local.ApiMovieRepositoryFake
import com.aplus.feature.detail.repository.remote.GenresRepositoryFake
import com.aplus.feature.detail.repository.remote.MoviesRepositoryFake
...
class DetailViewModelTest {
    ...
    @Test
    fun `when getSimilar is called observe with stateflow when api is success`() {
        runTest {
    ...
    @Test
    fun `when getSimilar is called observe with stateflow when api is failed`() {
    ...
    @Test
    fun `when getVideos is called observe with stateflow when api is success`() {
        runTest {
            viewModel.getVideos(1)
            assertEquals(
                Resource.success(fakeApiMovieRepository.dataVideos.body()?.results),
    ...
    @Test
    fun `when getVideos is called observe with stateflow when api is failed`() {
        val errorResponseVideos =
            "{ Unauthorized }".toResponseBody("application/json".toMediaTypeOrNull())
        val errorResourceVideos = Response.error<VideosResponse>(401, errorResponseVideos)

        fakeApiMovieRepository.dataVideos = errorResourceVideos
        apiMovieUseCases = ApiMovieUseCases(
            getGenresApi = GetGenresApi(fakeApiMovieRepository),
            getMoviesApi = GetMoviesApi(fakeApiMovieRepository),
            getNowPlayingApi = GetNowPlayingApi(fakeApiMovieRepository),
        ...
        runTest {
            viewModel.getVideos(1)
            assertEquals(
                Resource.error(errorResourceVideos.message(), null),
                viewModel.videos.value
            )
        }
    }
    ...
    @Test
    fun `when getReviews is called observe with stateflow when api is success`() {
        runTest {
            viewModel.getReview(1)
            assertEquals(
                Resource.success(fakeApiMovieRepository.dataReviews.body()?.results),
    ...
    @Test
    fun `when getReviews is called observe with stateflow when api is failed`() {
        val errorResponseReviews =
            "{ Unauthorized }".toResponseBody("application/json".toMediaTypeOrNull())
        val errorResourceReviews = Response.error<ReviewsResponse>(401, errorResponseReviews)

        fakeApiMovieRepository.dataReviews = errorResourceReviews
        apiMovieUseCases = ApiMovieUseCases(
            getGenresApi = GetGenresApi(fakeApiMovieRepository),
            getMoviesApi = GetMoviesApi(fakeApiMovieRepository),
            getNowPlayingApi = GetNowPlayingApi(fakeApiMovieRepository),
        ...
        runTest {
            viewModel.getReview(1)
            assertEquals(
                Resource.error(errorResourceReviews.message(), null),
                viewModel.reviews.value
            )
        }
    }
